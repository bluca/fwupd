...
static void
fu_uefi_kek_ignore_pk_child_func(void)
{
	gboolean ret;
	FuDevice *child;
	g_autoptr(FuContext) ctx = fu_context_new_full(FU_CONTEXT_FLAG_NO_QUIRKS);
	...
	g_autoptr(FuDevice) device_kek_child = NULL;
	g_autoptr(FuDevice) device_pk = NULL;
	g_autoptr(FuDevice) device_pk_child = NULL;
	g_autoptr(FuPlugin) plugin = NULL;
	g_autoptr(FuProgress) progress = fu_progress_new(G_STRLOC);
	g_autoptr(GError) error = NULL;
	...
	/* The parent and its X.509 children share a plugin name. Only the parent carries
	 * FuUefiDevice private flags and may be cached as the PK device. */
	device_pk = FU_DEVICE(fu_uefi_device_new(ctx, FU_EFIVARS_GUID_EFI_GLOBAL, "PK"));
	fu_device_set_plugin(device_pk, "uefi_pk");
	fu_device_add_private_flag(device_pk, FU_UEFI_DEVICE_PRIVATE_FLAG_IS_EXTERNAL);
	fu_plugin_runner_device_register(plugin, device_pk);

	device_pk_child = fu_device_new(ctx);
	fu_device_set_plugin(device_pk_child, "uefi_pk");
	fu_plugin_runner_device_register(plugin, device_pk_child);
...
main(int argc, char **argv)
{
	...
	g_test_add_func("/uefi-kek/ignore-pk-child", fu_uefi_kek_ignore_pk_child_func);
	return g_test_run();
}
