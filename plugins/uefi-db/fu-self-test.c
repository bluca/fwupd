...
static void
fu_uefi_db_ignore_kek_child_func(void)
{
	gboolean ret;
	FuDevice *child;
	g_autoptr(FuContext) ctx = fu_context_new_full(FU_CONTEXT_FLAG_NO_QUIRKS);
	 ...
	 * FuUefiDevice private flags and may be cached as the KEK device. */
	device_kek = FU_DEVICE(fu_uefi_device_new(ctx, FU_EFIVARS_GUID_EFI_GLOBAL, "KEK"));
	fu_device_set_plugin(device_kek, "uefi_kek");
	fu_device_add_private_flag(device_kek, FU_UEFI_DEVICE_PRIVATE_FLAG_IS_EXTERNAL);
	fu_plugin_runner_device_register(plugin, device_kek);
	...
	fu_device_set_plugin(device_kek_child, "uefi_kek");
	fu_plugin_runner_device_register(plugin, device_kek_child);
...
main(int argc, char **argv)
{
	...
	g_test_add_func("/uefi-db/ignore-kek-child", fu_uefi_db_ignore_kek_child_func);
